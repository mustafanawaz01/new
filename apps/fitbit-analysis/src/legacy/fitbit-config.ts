...
const legacyEnvSchema = z.object({
  FITBIT_CLIENT_ID: z.string().min(1, "FITBIT_CLIENT_ID required for legacy — see docs/legacy/"),
...
export type LegacyFitbitConfig = z.infer<typeof legacyEnvSchema>;

export function loadLegacyFitbitConfig(): LegacyFitbitConfig {
  const parsed = legacyEnvSchema.safeParse(process.env);
  if (!parsed.success) {
    const message = parsed.error.issues.map((i) => `  - ${i.path.join(".")}: ${i.message}`).join("\n");
    throw new Error(`Invalid legacy Fitbit environment:\n${message}`);
  }
  return parsed.data;
}
...
export const legacyPaths = {
  ...
  spikeOutputDir: resolve(packageRoot, "spike-output-legacy"),
} as const;
